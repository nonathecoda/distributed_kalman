...
class Main():
    ...
    def __init__(self, path = None, n_frames = 5, n_cameras = 1) -> None:
        self.start_time = time.monotonic()
        self.path = path
        if self.path is None:
            self.path = np.array(list(self.gen_path(n_frames))).T # (3, N)
        self.n_frames = n_frames
        self.cameras = self.create_cameras(n_cameras, path[:, 0])
    ...
    def update_world(self, num, path, line):
        ...
        if num > 1:
            self.target.set_acceleration(path[:, num] - 2*path[:, num-1] + path[:, num-2]) #TODO: fix this (divide by time!!)
        ...
        if num%5 == 0: # take measurement every 3 frames
            ...
            for cam in self.cameras:
                ...
                filtered_pose = cam.imm.update_pose(timestep = timestep)
    ...
    def create_cameras(self, n_cameras, initial_pose):
        cameras = []
        for i in range(n_cameras):
            cameras.append(Camera("camera "+str(i), 5, np.array([0, 0, 0, 0, 0, 0]), initial_pose))
        return cameras
...
if __name__ == '__main__':
    ...
    Main(path = path, n_cameras = n_cameras, n_frames = path.shape[1])